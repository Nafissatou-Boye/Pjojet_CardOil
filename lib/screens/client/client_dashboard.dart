...
class _DashboardHomeState extends State<DashboardHome> {
  UserModel? _user;
  CardModel? _card;        // ✅ données carte API
  bool _loading = true;
  String? _error;
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(children: [
          Positioned.fill(
            child: CustomPaint(painter: WavePainter(color: Colors.white, opacity: 0.08)),
          ),
          Positioned(
            right: -40, top: -40,
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Text(
                  '${t.status} : ${isVerified ? t.statusVerified : t.statusNotVerified}',
...
class _WhiteContent extends StatelessWidget {
      ...
      child: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(t.promotions, style: const TextStyle(
                fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1F2937))),
            const SizedBox(height: 12),

            FutureBuilder<List<PromotionModel>>(
              future: promotionService.getPromotions(user.selectedCompagnie, limit: 3),
              builder: (ctx, snap) {
                if (snap.hasData && snap.data!.isNotEmpty) {
                  return SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) => _buildPromoCard(snap.data![i], t),
                    ),
                  );
                }
                return Container(
                  ...
                  child: Center(child: Text(t.noPromotion, style: const TextStyle(color: Color(0xFF9CA3AF)))),
                );
              },
            ),
  ...
  Widget _buildServicesGrid(AppLocalizations t) {
    return Column(children: [
      Row(children: [
        Expanded(child: _ServiceCard(label: t.wash, icon: Icons.local_car_wash,
            colors: const [Color(0xFF0EA5E9), Color(0xFF06B6D4)])),
        const SizedBox(width: 12),
        Expanded(child: _ServiceCard(label: t.fuelMaintenance, icon: Icons.build_rounded,
            colors: const [Color(0xFFF59E0B), Color(0xFFFBBF24)])),
      ]),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: _ServiceCard(label: t.fuel, icon: Icons.local_gas_station,
        ...
        Expanded(
          child: Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.more_horiz, color: Color(0xFF6B7280), size: 20),
              const SizedBox(width: 6),
              Text(t.other, textAlign: TextAlign.center,
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF374151), height: 1.3)),
  ...
  Widget _buildPromoCard(PromotionModel promo, AppLocalizations t) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(promo.title, style: const TextStyle(color: Color(0xFF1F2937), fontSize: 15, fontWeight: FontWeight.bold)),
        const SizedBox(height: 6),
        Expanded(child: Text(promo.description, maxLines: 2, overflow: TextOverflow.ellipsis,
            style: const TextStyle(color: Color(0xFF374151), fontSize: 12))),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Text(t.seeAll, style: TextStyle(color: colors[0], fontSize: 12, fontWeight: FontWeight.bold)),
            const SizedBox(width: 4),
...
class _ServiceCard extends StatelessWidget {
  ...
  const _ServiceCard({required this.label, required this.icon, required this.colors});
    ...
    return Container(
      ...
      child: Stack(clipBehavior: Clip.hardEdge, children: [
        Positioned(right: -12, bottom: -12,
            child: Icon(icon, size: 72, color: Colors.white.withOpacity(0.18))),
        Padding(
